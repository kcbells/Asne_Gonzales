...
/**
 * Helper to apply a lump sum payment to unpaid schedules in chronological order.
 */
...
if (isset($_POST['action']) && $_POST['action'] === "process_pay_all") {
    ...
    try {
        ...
        if ($applied <= 0) throw new Exception("No unpaid schedules found.");

        $payStmt = $conn->prepare("INSERT INTO payments (rent_id, type, amount, datetime_paid, method, status) VALUES (?, 'monthly', ?, ?, ?, 'success')");
...
$pending_sql = "
    (SELECT 
        au.assigned_units_id AS id, 'downpayment' AS pay_type, t.firstname, t.lastname, u.unit_number, 
        au.downpayment AS amount, au.start_date AS due_date, 0 AS overdue_count, 0 AS overdue_total
    ...
    (SELECT 
        ...
        SUM(ps.amount_due) AS amount, MIN(ps.due_date) AS due_date, COUNT(*) AS overdue_count, SUM(ps.amount_due) AS overdue_total
