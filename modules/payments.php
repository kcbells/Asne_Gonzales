...
if (isset($_POST['action']) && $_POST['action'] == "process_payment") {
    ...
    try {
        ...
        // ✅ 1) If downpayment, check FIRST (before inserting payment)
        if ($type == 'downpayment') {
            $dpCheck = $conn->prepare("
            SELECT COUNT(*)
            FROM payments
            WHERE rent_id = ? AND type = 'downpayment' AND status = 'success'
        ");
        ...
        // ✅ 2) Record payment in history (runs after check)
        $stmt = $conn->prepare("
        INSERT INTO payments (rent_id, type, amount, datetime_paid, method, status)
        VALUES (?, ?, ?, ?, ?, 'success')
    ");
        $stmt->bind_param("isdss", $rent_id, $type, $amount, $date_paid, $method);
        $stmt->execute();

        // ✅ 3) Continue your existing logic
        if ($type == 'downpayment') {

            // (optional but recommended) save actual paid downpayment into assigned_units
        ...
        ");
            ...
            // Prevent duplicate schedule for this rent_id
            ...
            ");
                ...
                $m_rent = floatval(str_replace(',', '', $info['monthly_rent']));
            ...
            ");
                ...
                for ($i = 1; $i <= 12; $i++) {
                    ...
                    $schStmt->bind_param("isd", $rent_id, $due_date, $m_rent);
                    $schStmt->execute();
                }
                $schStmt->close();
            }
...
// --- 2. DATA FETCHING ---
$pending_sql = "(SELECT au.assigned_units_id as id,
        NULL as sched_id,
    ...
    JOIN units u ON au.unit_id = u.unit_id 
    WHERE au.status IN ('pending downpayment'))
    UNION ALL
    (SELECT 
        au.assigned_units_id as id,
        ps.schedule_id as sched_id,
        'monthly' as pay_type,
        t.firstname,
        t.lastname,
        u.unit_number,
...
$history_sql = "SELECT p.*, t.firstname, t.lastname, u.unit_number, prop.property_name 
    FROM payments p
...
<div class="container-fluid py-4 bg-light min-vh-100">
    ...
    <ul class="nav nav-pills mb-4 bg-white p-2 rounded shadow-sm" id="payTabs" style="width: fit-content;">
        <li class="nav-item">
            <button class="nav-link active fw-bold px-4" data-bs-toggle="pill"
                data-bs-target="#tab-pending">Pending</button>
        </li>
        <li class="nav-item">
            <button class="nav-link fw-bold px-4 text-secondary" data-bs-toggle="pill"
                        ...
                        <tbody>
                            <?php while ($p = $pending_list->fetch_assoc()):
                                ...
                                <tr class="bg-white">
                                    <td class="ps-4">
                                        <span
                                            class="badge rounded-pill py-2 px-3 <?= $p['pay_type'] == 'downpayment' ? 'bg-info-subtle text-info border border-info-subtle' : 'bg-warning-subtle text-warning border border-warning-subtle' ?>">
                                    ...
                                    <td class="text-end pe-4">
                                        <button class="btn btn-outline-primary btn-sm rounded-pill px-3 fw-bold"
                                            data-bs-toggle="modal" data-bs-target="#payModal<?= $unique_id ?>">Confirm
                                            Pay</button>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                    ...
                    <table class="table align-middle mb-0">
                        ...
                        <tbody>
                            <?php while ($h = $history_list->fetch_assoc()): ?>
                                <tr class="bg-white">
                                    <td class="ps-4 text-muted small"><?= date('M d, Y', strtotime($h['datetime_paid'])) ?>
                                    </td>
                                    <td class="fw-bold text-dark"><?= $h['lastname'] ?>, <?= $h['firstname'] ?></td>
                                    <td class="text-secondary small"><?= $h['property_name'] ?> - Unit
                                        <?= $h['unit_number'] ?>
                                    </td>
                                    <td><span
                                            class="badge bg-light text-secondary border px-3"><?= ucfirst($h['type']) ?></span>
                                    </td>
                                    <td class="text-end pe-4 fw-bold text-success">₱<?= number_format($h['amount'], 2) ?>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        </tbody>
            ...
            <form method="POST" class="modal-content border-0 shadow-lg">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="form-text text-center mt-2 small">You can re-type the amount if it differs from the
                            expected.</div>
                    </div>
